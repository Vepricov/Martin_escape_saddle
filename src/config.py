...
class DataTrainingArguments:
    ...
    n_samples: Optional[int] = field(
        default=None, 
        metadata={"help": "Number of samples for sclearn make_classification dataset"}
    )
    n_informative: Optional[int] = field(
        default=None, 
        metadata={"help": "Number of informative features for sclearn make_classification dataset"}
    )
    n_classes: Optional[int] = field(
        default=None, 
        metadata={"help": "Number of classes features for sclearn make_classification dataset"}
    )
    ...
    def __post_init__(self):
        if self.task_name is not None:
            ...
            if self.dataset_name == "glue" and self.task_name not in task_to_keys.keys():
                raise ValueError("Unknown task, you should pick one in " + ",".join(task_to_keys.keys()))
            if self.dataset_name == "libsvm" and self.task_name not in ["make_classification",
                                                                        "mushrooms",
                                                                        "binary"]:
                raise ValueError("Unknown task, you should pick one in " + ",".join(["make_classification", "mushrooms", "binary"]))
...
class ModelArguments:
    ...
    hidden_dim:  Optional[int] = field(
        default=None,
        metadata={"help": "Hidden dim of the Linear Classifier"},
    )
...
class TrainingArguments(TrainingArguments):
    ...
    num_epochs: Optional[int] = field(
        default=10,
        metadata={"help": "Num epochs to train"}
    )
    optimizer_name: Optional[str] = field(
        default="Adam",
        metadata={"help": "Name of the optimizer"}
    )
    ...
    train_len: Optional[int] = field(
        default=None, 
        metadata={"help": "Len of train dataset. For wandb"}
    )
    eval_len: Optional[int] = field(
        default=None, 
        metadata={"help": "Len of eval dataset. For wandb"}
    )
